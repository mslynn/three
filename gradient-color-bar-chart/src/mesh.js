...
function createBar(dataArr) {
  ...
  bars.add(create(dataArr));
  bars.add(createNum(dataArr));

  return bars;
}
function create(dataArr1) {
  const nums1 = new THREE.Group();
  dataArr1.forEach((item, i) => {
    const geometry = new THREE.PlaneGeometry(10, item, 1, 10);
